...
class RNN_Model(object):
    ...
    def __init__(self,config,is_training=True):
        ...
        lstm_cell = rnn.BasicLSTMCell(hidden_neural_size,forget_bias=0.0,state_is_tuple=True) # if the state_is_tuple is True it means the states are 2-tuples
        ...
        if self.keep_prob<1:
            lstm_cell =  rnn.DropoutWrapper(lstm_cell,output_keep_prob=self.keep_prob) #lstm_cell =  tf.nn.rnn_cell.DropoutWrapper

        #cell = tf.nn.rnn_cell.MultiRNNCell([lstm_cell]*hidden_layer_num,state_is_tuple=True)
        cell = rnn.MultiRNNCell([lstm_cell]*hidden_layer_num,state_is_tuple=True)
        ...
        with tf.device("/cpu:0"),tf.name_scope("embedding_layer"):
            embedding = tf.get_variable("embedding",[vocabulary_size,embed_dim],dtype=tf.float32)  #对于get_variable()，来说，如果已经创建的变量对象，就把那个对象返回，如果没有创建变量对象的话，就创建一个新的。
        ...
        with tf.name_scope("Softmax_layer_and_output"):
            ...
            self.logits = tf.matmul(out_put,softmax_w)+softmax_b  # y=wx+b
